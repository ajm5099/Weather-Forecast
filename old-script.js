...
function getCurrentWeather() {
    $.ajax({
        ...
        var currentUvURL = "http://api.openweathermap.org/data/2.5/uvi?&lat=" + lat + "lon=" + long + "&appid=907c23e4987ec932c6e00aa83a52deef";
        $.ajax({
            url: currentUvURL,
            method: "GET"
        }).then(function (responseUV) {
            console.log(responseUV);
            var uvIdx = responseUV.value
            $("#uv").text("UV Index: " + uvIdx); //reuse above
        });
